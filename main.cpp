...
namespace
{
  ...
  void initSystem(int argc, char * argv[])
  {
	...
	current_sys = "PendulumSys";

	if (argc > 1){
		char* type_chosen = argv[1];
		switch(type_chosen[0]){
		case 'E':
		...
		timeStepper = new RK4();
		cout << "Default RK4" <<endl;
	}
  }
  // This is reset func for system
  void reset_system(){
	  delete system;
	  if ((current_sys.compare("SimpleSys"))==0){
		  system = new SimpleSystem();
	  }
	  if ((current_sys.compare("PendulumSys"))==0){
		  system = new PendulumSystem(8);
	  }
	  if ((current_sys.compare("ClothSys"))==0){
		  system = new ClothSystem(12);
	  }
  }
  ...
  void drawSystem()
  {
    ...
    void keyboardFunc( unsigned char key, int x, int y )
    {
		...
		case '3':{
			system = new ClothSystem(12);
			current_sys = "ClothSys";
			break;
		}
		case 'm':{
			if ((current_sys.compare("ClothSys"))==0){
				ClothSystem* clothSystem = dynamic_cast<ClothSystem*>(system);
				if (clothSystem != NULL) {
					clothSystem->Move_button();
					string mvr= clothSystem->Move_String;
					cout << mvr<<endl;
					break;
				}
			}
		...
		case 'o':{
			if ((current_sys.compare("PendulumSys"))==0){
				PendulumSystem* pls = dynamic_cast<PendulumSystem*>(system);
				if (pls != NULL) {
					pls->windToggle();
					break;
				}
...
int main( int argc, char* argv[] )
{
    ...
    // Set up the callback function fors resizing windows
